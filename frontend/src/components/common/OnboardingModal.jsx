  ...
  return (
    <div className="flex flex-col items-center text-center gap-5">
      ...
      <div>
        ...
        <p className="text-zinc-400 text-sm">Trading Calculator PRO</p>
      </div>
      <p className="text-zinc-300 text-sm leading-relaxed max-w-xs">
        Tienes acceso a más de{" "}
        <span className="text-green-400 font-semibold">
          8 calculadoras profesionales
        </span>{" "}
        diseñadas para mejorar tu gestión de riesgo y tomar mejores decisiones
        de trading.
      </p>
      <div className="flex items-center justify-center gap-6 mt-2">
        <div className="flex flex-col items-center gap-1.5">
          ...
          <span className="text-xs text-zinc-400">Calcular</span>
        </div>
        <div className="flex flex-col items-center gap-1.5">
          <div className="w-10 h-10 rounded-lg bg-zinc-800 border border-zinc-700 flex items-center justify-center">
            <BarChart3 className="w-5 h-5 text-green-400" />
          </div>
          <span className="text-xs text-zinc-400">Analizar</span>
        </div>
        <div className="flex flex-col items-center gap-1.5">
          <div className="w-10 h-10 rounded-lg bg-zinc-800 border border-zinc-700 flex items-center justify-center">
            <BookOpen className="w-5 h-5 text-green-400" />
          </div>
          <span className="text-xs text-zinc-400">Registrar</span>
        </div>
      </div>
    </div>
  );
...
function Step2() {
  return (
    <div className="flex flex-col items-center text-center gap-5">
      ...
      <div>
        <h2 className="text-xl font-bold text-white mb-1">Calculadoras</h2>
        <p className="text-zinc-400 text-sm">Tu arsenal de trading</p>
      </div>
      <p className="text-zinc-300 text-sm leading-relaxed max-w-xs">
        Herramientas profesionales para calcular tamaño de posición, simular
        estrategias y gestionar tu riesgo con precisión.
      </p>
...
const TABS = [
  { icon: Calculator, label: "Calculadoras", desc: "8+ herramientas" },
  { icon: BookOpen, label: "Diario de Trading", desc: "Registra operaciones" },
  { icon: Bell, label: "Alertas de Precio", desc: "Notificaciones clave" },
  { icon: BarChart3, label: "Historial", desc: "Analiza tu progreso" },
];
  ...
  return (
    <div className="flex flex-col items-center text-center gap-5">
      ...
      <div>
        ...
        <p className="text-zinc-400 text-sm">Todo en un solo lugar</p>
      </div>
      <p className="text-zinc-300 text-sm leading-relaxed max-w-xs">
        El panel central organiza todas tus herramientas en pestañas fáciles de
        navegar.
      </p>
  ...
  return (
    <div className="flex flex-col items-center text-center gap-5">
      ...
      <div>
        ...
        <p className="text-zinc-400 text-sm">Tu cuenta está configurada</p>
      </div>
      <p className="text-zinc-300 text-sm leading-relaxed max-w-xs">
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) complete(); }}>
      <DialogContent className="max-w-md bg-zinc-900 border border-zinc-700 text-white p-6 gap-0 [&>button]:text-zinc-400 [&>button]:hover:text-white">
        ...
        <div className="min-h-[340px] flex flex-col justify-center">
          {step === 0 && <Step1 userName={user?.name} />}
          {step === 1 && <Step2 />}
          {step === 2 && <Step3 />}
          {step === 3 && <Step4 onComplete={complete} />}
        </div>

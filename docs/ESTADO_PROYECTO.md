...
>
> - 📅 **Última verificación real contra el código:** 2026-09-04 (CI: ocho ejecuciones
>   seguidas en rojo por un solo paso, arreglado de raíz — BUG-081/G-42. Además, el
>   suite del shim ejecutándose de verdad —44 pasan, antes 42 se saltaban en silencio—
>   y el banco E2E corrido entero contra backend y Postgres reales. Se cerraron tres
>   filas de §3 que daban por abierto lo que el código ya tenía hecho: BUG-007, G-26 y
>   G-38). La verificación integral anterior: 2026-08-26 (examen integral:
...
> - 🌿 **Rama de trabajo actual:** `claude/web-excellence-improvement-iybap1`
...
| **Panel de ajustes (`/settings`)** | 🟢 | Rediseñado el 2026-09-03: de ocho tarjetas apiladas a **consola con rail de seis secciones**, enlazables (`?s=seguridad`). Incorpora tema, idioma y la cuenta de la mesa, que antes vivían fuera. Las seis propuestas comparadas y por qué ganó ésta, en [`maquetas/`](./maquetas/README.md) y [`DECISIONES.md`](./DECISIONES.md) |
...
| **CI de PR (`ci.yml`)** | 🟢 | Backend: `py_compile *.py` + pytest. Frontend: i18n + credentials + engine + lint + build. **Doc: `gen-mapa --check` + `check-doc-links` + paridad del catálogo** (añadido 2026-08-13, cierra G-18). ⚠️ **Esta fila dijo 🟢 mientras CI llevaba ocho ejecuciones seguidas en ROJO** (BUG-081, arreglado el 2026-09-04): el único paso que fallaba era `csp.js`, porque el job compilaba sin `REACT_APP_BACKEND_URL` y el marcador sobrevivía dentro de `connect-src`. Un semáforo escrito a mano no ve el color de la última ejecución: antes de fiarse de esta fila, mírala en Actions |
...
| BUG-007 | ~~Preferencias de usuario solo en `localStorage` (no cross-device)~~ | 🟢 | ✅ **Cerrado — la fila estaba desfasada desde el 2026-08-08** (lo cerró G-25 y esta línea siguió diciendo lo contrario un mes). No hay endpoint `/user/preferences` y no hace falta: viajan en `user_states` bajo el documento `preferences_v1` (`lib/cloudPrefs.js:47`, `POST /user-states/save` en `server.py:5605`). Verificado contra el código el 2026-09-04 |
...
| G-26 | ~~**No se puede editar el perfil.** No existe `PUT /auth/profile` ni pantalla~~ | 🟢 | ✅ **Cerrado (2026-09-01, BUG-079)**, y esta fila seguía abierta: `server.py:3720-3721` registra el endpoint con `POST` **y** `PUT`, y lo llaman `store.js:227` y `SettingsPage.jsx:113`. Verificado contra el código el 2026-09-04 |
...
| G-38 | ~~**El shim `Collection` sigue sin tests (era G-17)**~~ | 🟢 | ✅ **Cerrado con G-17 el 2026-08-27** y esta fila se quedó en 🟠 diciendo lo contrario: son **44 pruebas** contra PostgreSQL real en `test_shim_collection_unit.py`. El 2026-09-04 se arregló además que 42 de ellas se SALTABAN en silencio fuera de CI (el rol salía de `$USER`, que no está exportado en un contenedor, y la autenticación por socket es `peer`). Lo que sigue abierto es lo que dependía de esto: partir `server.py` (BUG-008) |
| G-39 | ~~**El escape hatch de 2FA para admins en desarrollo sólo existe en el backend; el frontend no sabe que existe.**~~ `ProtectedRoute.jsx` redirigía a `/settings` a todo admin con `two_factor_enabled === false` **sin mirar el entorno**, así que expulsaba también a quien el servidor sí iba a dejar pasar | 🟢 | ✅ **Cerrado el 2026-08-31**, y resultó ser la mitad barata de algo peor (BUG-076): la otra mitad encerraba al administrador **en producción**. La guarda se retira: el frontend no conoce ni `ADMIN_2FA_OPTIONAL` ni el margen de alta nuevo, así que no puede decidir. Manda el **428**, que `AdminPage` ya traduce a un mensaje y una redirección desde BUG-073 — una sola fuente de verdad, y es la que tiene los datos. Probar `/admin` en local ya no exige fabricar un TOTP real |
| G-40 | **El escáner sólo detecta patrones de vela — 0 patrones chartistas en vivo.** Comparado con TrendSpider (226 patrones: 172 de velas + 54 chartistas, medido 2026-08-31), el escáner llegó a 35 velas pero los 42 patrones chartistas de la Academia (H-C-H, triángulos, banderas, cuñas, cup&handle…) son sólo ilustraciones educativas: nadie los detecta sobre un símbolo real. El diseño **ya existe y está detallado**, sin construir: `docs/DETALLE_TECNICAS_IMPLEMENTACION.md` Lote 5 §5.2 — ~18-20 patrones por ajuste geométrico sobre `detect_swings`, con objetivo medido por fórmula y estadística de Bulkowski, fichero `chart_patterns.py` y endpoint `/education/chart-pattern-scan/{symbol}` ya nombrados | 🟠 | En cola de ejecución, a petición del dueño (2026-08-31). Construir el detector geométrico + tests + integración frontend (reutiliza `ChartPatternFigure`, las 42 SVG ya existen) siguiendo exactamente el Lote 5. Es un proyecto de varias horas, no un ajuste |
| G-41 | ~~**`probar()` de `probar-verificadores.sh` no comprueba que su sabotaje se haya aplicado.**~~ Ejecutaba `eval "$sabotaje" >/dev/null 2>&1` **descartando el código de salida**, así que un sabotaje muerto dejaba el fichero intacto y el informe acusaba al verificador de no verificar nada | 🟢 | ✅ **Cerrado el 2026-08-31**, en la misma sesión que lo abrió. Un sabotaje que falla se reporta con su error y cuenta como fallo **del test**, no del producto; igual el cebo de `probar_inverso()`. Aparecieron **dos** formas de morir en silencio, no una: ocho heredocs con `IndentationError` y un ancla que el código había dejado atrás. La segunda no se habría cazado dedentando: hacía falta mirar el código de salida |
...
| G-42 | ~~**CI en rojo en todos los PR, y el semáforo decía 🟢.**~~ El job de frontend compilaba sin `REACT_APP_BACKEND_URL`, el marcador `%REACT_APP_BACKEND_URL%` sobrevivía dentro de `connect-src` y el navegador descartaba esa fuente, así que `csp.js` fallaba en las doce pantallas. Ocho ejecuciones seguidas en rojo por algo que no tenía que ver con ningún diff | 🟢 | ✅ **Cerrado el 2026-09-04 (BUG-081)**: CI compila con el backend local, `check-csp-origenes.js` exige el par `http(s)`+`ws(s)` en la política compilada —también en el DESPLIEGUE, donde un secreto vacío publicaba sin que nada mirara—, `csp.js` ya no confunde el fallback de la SPA con una página estática, y su bloque de WebSocket dice cuándo NO se ha comprobado |

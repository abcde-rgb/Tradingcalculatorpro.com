...
- **Analítica/SEO**: GA4 + GTM + GSC/Bing/Yandex + IndexNow, `sitemap.xml` (2.475
  ...
  **2.410 páginas estáticas** que genera el `postbuild` (13 calculadoras, 75 temas
  ...
  `markets`, `strategies`, `patterns`, `candles`), y **10 rutas del SPA con
  fichero propio**: las cuatro indexables (`pricing`, `about`, `contact`,
  `legal`), las tres premium (`education`, `options`, `options/strategies`) y las
  tres de acceso (`login`, `register`, `forgot-password`). Las seis últimas
  responden 200 con `noindex, follow` y NO van al sitemap — tienen fichero para
  que los miles de enlaces internos que las citan no acaben en un 404, y no se
  bloquean en `robots.txt` porque una ruta prohibida ahí nunca llega a leer su
  propio `noindex`. Sin ese fichero GitHub Pages les devolvía 404 a los
  buscadores (BUG-081 y BUG-086). `lastmod` sale de
...
| G-42 | **5 de los 75 temas de la academia siguen publicándose delgados: 43 páginas.** Los otros 70 pasaron de ~94 a ~453 palabras leyendo la estructura que la propia aplicación pinta (`tradingEducationContent.js`), con el emparejamiento tema→getter descubierto por la clave del título, no escrito a mano. Estos 5 —`session-phases`, `strategies`, `harmonic-patterns`, `time-impact`, `pre-trade-protocol`— **no tienen estructura de datos que leer**: se pintan con un componente JSX propio (`SessionPhasesTimeline`, `TimeVsImpact`, `PreTradeProtocol`…) que lleva el contenido dentro, así que ni el getter ni el convenio de claves encuentran nada. Medido el 2026-09-03 sobre el build | 🟡 | Sacar el contenido de esos 5 componentes a `tradingEducationContent.js`, que es donde vive el del resto. Arregla el SEO y de paso deja de haber dos sitios donde puede vivir el contenido de un módulo. **Comprobar cada uno contra lo que pinta el componente**: publicar conceptos que no son los del tema es peor que publicar pocos |
...
| G-43 | **80 de las 100 fichas de mercado publican texto en inglés bajo `<html lang>` de otro idioma.** `gen-seo-pages.js` cae a inglés cuando `marketTypesContent.js` no trae la ficha en ese idioma —decisión escrita, con el criterio «el schema y lo visible coinciden»—, pero eso deja `/ja/markets/forex/` con `<html lang="ja">`, `hreflang="ja"` e `inLanguage:"ja"` sobre texto inglés. Google colapsa el grupo de diez, gasta rastreo y el dato estructurado no describe el contenido. Dos salidas: traducir las nueve fichas, o dejar de generar el idioma sin traducción (que es lo que ya hacen calculadoras y academia: `if (!d \|\| !d.title) return;`). La segunda retira 80 URLs publicadas | 🟠 | ⬜ Abierto — decisión de tráfico, no de código |
| G-44 | ~~**Las 1.640 páginas estáticas son una isla sin enlaces entrantes.**~~ No había una sola referencia a `/tools/`, `/learn/` o `/markets/` en `frontend/src`: la SPA nunca enlazaba al mayor activo de captación del proyecto. Google llegaba por el sitemap —que DESCUBRE una URL pero no reparte autoridad— y por los cinco enlaces del `<noscript>` del shell, y nada más | 🟢 | ✅ **Cerrado el 2026-09-05**: **60 hubs** de sección (`learn`, `tools`, `markets`, `strategies`, `patterns`, `candles` × 10 idiomas) con `CollectionPage` + `ItemList`, enlazados desde el pie de la SPA —con `<a>`, no con `<Link>`: son HTML estático— y desde el `<noscript>` del shell. Toda página queda a **dos saltos de la portada**. `check-seo.js` comprueba que ninguna se queda sin hub que la enlace, y tiene su sabotaje en `probar-verificadores.sh` . **⚠️ Corregido el 2026-09-06 (BUG-090)**: el «toda página queda a dos saltos de la portada» era cierto DENTRO de cada idioma y falso ENTRE idiomas. Medido con `auditar-visibilidad.js`: sólo **182 de 1.811** páginas se alcanzaban siguiendo `<a>` desde la portada, y las 182 eran españolas; los otros nueve idiomas no recibían ni un enlace. El `hreflang` no lo tapaba —descubre y agrupa, no reparte autoridad— y la comprobación de huérfanas tampoco, porque es local a cada idioma. Cerrado de verdad con el selector de idioma visible en las tres plantillas |

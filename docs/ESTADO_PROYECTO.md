...
| G-37 | **La región de despliegue se contradice dentro del propio checklist.** `DEPLOY_CHECKLIST.md` dice `us-east1` en la tabla de despliegue (líneas 14/27/43) y `europe-west1` en el bloque de infraestructura (83-85); la skill `pre-deploy` repetía la segunda. Un `gcloud run services describe --region=europe-west1` devuelve «not found» sobre un servicio que existe | 🟠 | **Sólo se cierra en la consola de GCP.** Si Cloud SQL está de verdad en `europe-west1` con Cloud Run en `us-east1`, cada consulta cruza el Atlántico. Marcado en el checklist como pendiente de verificar, no como verdad |

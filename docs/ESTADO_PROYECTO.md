...
| **Comunidad (`/community`)** | 🟠 | **Construida el 2026-09-03, sin desplegar.** Foro propio sobre FastAPI + el shim (`backend/forum.py`, 19 rutas) y React (`CommunityPage`): hilos por activo y producto, orden por actividad/nuevos/tendencia/vistas/votos/respuestas, seguimiento entre miembros con sus hilos primero, **seudónimo obligatorio** (la API no devuelve correo, nombre ni `user_id`), análisis adjunto con el **R:R calculado en el servidor** y `null` cuando falta el stop, traducción bajo demanda y cola de moderación en el panel admin. Se lee sin cuenta. **Arranca vacía a propósito**: cero contenido sembrado. 96 tests offline + sonda de API contra PostgreSQL + sonda de navegador. Falta desplegar, notificaciones, antispam y buscador — ver [`FORO_COMUNIDAD.md`](./FORO_COMUNIDAD.md) § 8 |

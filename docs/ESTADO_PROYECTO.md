...
| G-41 | **`probar()` de `probar-verificadores.sh` no comprueba que su sabotaje se haya aplicado.** Ejecuta `eval "$sabotaje" >/dev/null 2>&1` y **descarta el código de salida**, así que un sabotaje que muere —un `IndentationError`, un ancla que ya no existe— deja el fichero intacto, el verificador sigue pasando y el informe dice «SOBREVIVE al sabotaje», que es el mensaje de un verificador roto. Ocho sabotajes llevaban así desde que se escribieron (BUG-078, 2026-08-31): un heredoc de Python con el cuerpo indentado dos espacios. Los dos síntomas son indistinguibles desde el informe, y eso es justo lo que este script existe para no permitir | 🟠 | Exigir que el sabotaje salga con 0 y, si no, marcarlo como fallo del propio test con su mensaje de error. Es un cambio de cinco líneas en `probar()` y `probar_inverso()`; lo que cuesta es revisar los sabotajes que hoy pasan por buenos porque nunca se aplicaron |

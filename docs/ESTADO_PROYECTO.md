...
| G-42 | ~~**El «Monitor de Errores» del panel de admin leía una tabla que no escribía nadie.**~~ `GET /admin/errors` y su tarjeta existían desde el principio; el `componentDidCatch` del `ErrorBoundary` sólo hacía `console.error` y dejaba un *«Wire up Sentry here»* sin cablear. `error_logs` no recibió una fila en toda la vida del proyecto, y la tarjeta decía «✓ Sin errores pendientes» tanto si la web iba fina como si se caía cada dos minutos | 🟢 | ✅ **Cerrado el 2026-09-02** (BUG-081). `POST /api/errors/report` + `reportarError.js`, cableado al `ErrorBoundary` y a `unhandledrejection`/`error` desde `index.js`. Agrupa por huella con contador `veces`, reabre lo que se dio por resuelto y vuelve a caer, redacta correos/tokens/números largos y **no guarda `user_id`** (sólo `autenticado`), así que no entra en las listas del RGPD. Contador de errores abiertos en la fila de métricas, visible desde todas las secciones. 28 tests, 13 sabotajes, verificado contra Postgres y el backend vivo |
...
- [x] ~~**Saber que la web se está cayendo sin que lo cuente un usuario** (G-42)~~ — cerrado el
      2026-09-02: las caídas del navegador llegan al panel de admin (Sistema → Monitor de
      Errores) agrupadas, y el contador de abiertos vive en la fila de métricas. Lo que
      **sigue sin cubrir**: los 500 del backend no entran en esa tabla — los ve `gcloud
      logging`, no el panel.

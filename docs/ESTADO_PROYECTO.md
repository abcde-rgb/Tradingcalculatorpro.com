...
| G-33 | **Las calculadoras sueltas y el catálogo.** El enunciado original —«las catorce siguen sin rehacer»— citaba dos fallos concretos: el `pipValuePerStandardLot = 10` de `LotSizeCalculator` y el `BTC` fijo de `PositionSizeCalculator`. El primero se arregló el 22-08; el segundo era peor de lo que decía la ficha: no era la etiqueta, era que **dimensionaba sin tamaño de contrato y sin convertir la divisa**, y mandaba al diario una cantidad en unidades donde el diario espera contratos | 🟡 | **Por dentro, cerrado (2026-08-26).** Ninguna calculadora que dé dinero dependiente del instrumento tiene ya cifras de instrumento dentro: las siete que lo necesitan salen de `lib/instruments.js` + `lib/deskMath.js` (`LotSize`, `PositionSize`, `PatternTrading`, `Futures`, `Leverage`, `MonteCarlo`, `CrossMargin`) y `PartialExit` pide el multiplicador explícitamente. `PositionSizeCalculator` daba **20** donde son 0,2 lotes de oro y **400** donde son 0,62 lotes de USDJPY. De paso salieron tres fallos que ningún test offline veía: `lotSizing` sin stop devolvía el máximo por exposición (las dos calculadoras pintaban el aviso Y una cifra), Black-Scholes valoraba con un 5 % escrito a mano mientras el backend usaba el tipo vivo, y la segunda calculadora de posición —la de la Academia— compartía `data-testid` con la del panel y pintaba `0` donde no se puede calcular. Cubierto por `engine-check` (16 comprobaciones con las cifras a mano), por `simulacion-masiva` a través de `lotSizing`, y por tres sabotajes nuevos. **Por fuera, pendiente**: quedan **12 etiquetas `($)`** y **5 `(USD)`** escritas a mano (`Leverage`, `MonteCarlo`, `Percentage`, `Spot`, `TargetPrice`, `PatternTrading`) que mienten en cualquier instrumento que no cotice en dólares, y colores fuera de los tokens en nueve ficheros —`Futures` con 22—. Y cinco calculadoras siguen con la matemática dentro del JSX (`Spot`, `TargetPrice`, `Percentage`, `Fibonacci`, `Compound`), aunque las cinco son aritmética sin instrumento y su resultado es correcto |

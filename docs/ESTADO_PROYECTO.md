...
| G-40 | **El escáner detecta patrones chartistas por API, pero ninguna pantalla los enseña.** Comparado con TrendSpider (226 patrones: 172 de velas + 54 chartistas, medido 2026-08-31), el escáner tenía 35 velas y 0 chartistas en vivo — los 42 patrones chartistas de la Academia eran sólo ilustraciones educativas. **Backend cerrado el 2026-09-06**: `chart_patterns.py` detecta 13 de los ~19-20 (H-C-H ±inverso, doble/triple techo-suelo, rectángulo, 3 triángulos, 2 cuñas, ensanche) por ajuste geométrico sobre `detect_swings`, siguiendo `docs/DETALLE_TECNICAS_IMPLEMENTACION.md` Lote 5 §5.1-5.2, con `GET /education/chart-pattern-catalog` y `GET /education/chart-pattern-scan/{symbol}` en pie. Verificado contra el propio ejemplo de la doc (H-C-H 110/120/111 → objetivo 80) en `test_chart_patterns_unit.py` (9 tests). `bulkowski` viaja `None` a propósito: no hay tabla citable a mano para las tasas de Bulkowski del Lote 5 §5.4, y un número de memoria mal recordado sigue siendo un dato inventado | 🟠 | **Sigue 🟠, no 🟢: falta la pantalla.** Overlay de las líneas sobre el gráfico + tarjeta reutilizando `ChartPatternFigure` (§5.5, las 42 SVG ya existen) + i18n ×8 en 10 idiomas. Quedan además 9 formas sin geometría de swings (Cup&Handle, banderas, High Tight Flag, Diamante, Bump-and-Run, V-spike, Scallop, Darvas Box, Rounding Bottom) documentadas como pendientes en el propio módulo, y las estadísticas de Bulkowski con fuente citable |

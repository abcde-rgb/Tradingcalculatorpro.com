...
| G-39 | **El escape hatch de 2FA para admins en desarrollo sólo existe en el backend; el frontend no sabe que existe.** `ADMIN_2FA_OPTIONAL` (`server.py:1402`) evita el 428 fuera de producción a propósito — es la decisión de seguridad que `AUDITORIA_2026-07-27.md` §3.2 elogia por no poder desactivarse en producción tocando una env var. Pero `ProtectedRoute.jsx:40` redirige cualquier admin sin `two_factor_enabled` a `/settings` **sin mirar en qué entorno está**, porque el frontend nunca recibe ese dato. Resultado: en local o en el sandbox de QA, un admin sembrado sin 2FA real no llega a ver `/admin` en el navegador aunque el backend ya se lo permitiría — hubo que generar y verificar un secreto TOTP real con `pyotp` sólo para poder mirar el panel durante el rediseño de navegación (2026-08-31). No es un fallo de seguridad (es más estricto de lo necesario, no menos), pero cuesta tiempo de verificación cada vez y nadie lo había dejado escrito | 🟠 | Sin decidir: o `/auth/me` manda un dato tipo `admin_2fa_required` y `ProtectedRoute` lo respeta, o se documenta en `qa`/`sembrar.py` que hace falta 2FA real para probar `/admin` en local y se automatiza sembrarlo con un secreto conocido |

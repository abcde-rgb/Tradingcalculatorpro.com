...
| G-28 | ~~**Se anuncia precio 0 a Google con muro de pago duro.**~~ `gen-seo-pages.js` emitía `offers: {price:'0', priceCurrency:'EUR'}` en las páginas de calculadora, con el título de una de ellas —Tamaño de Posición— diciendo «Gratis»/«Free», mientras `public/index.html` declara ofertas de 17/45/200 € y el CTA lleva a `/dashboard`, que exige suscripción activa | 🟢 | ✅ **Cerrado (2026-08-31)**: el bloque `offers` ya se había quitado en una sesión anterior (queda el comentario en `gen-seo-pages.js:439` explicando por qué: una calculadora suelta no tiene precio propio que declarar). Quedaba el título «Gratis y Profesional»/«Free & Professional» de Tamaño de Posición sin tocar — corregido a «Riesgo y Gestión de Capital»/«Risk & Money Management», coherente con el resto de calculadoras. Verificado con `npm run build` + `check-seo` sobre el build fresco: 1640 páginas, 0 apariciones de «Gratis»/«Free» contradictorias |

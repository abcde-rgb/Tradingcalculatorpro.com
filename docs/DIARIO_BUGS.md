...
| BUG-081 | **Las siete URLs más caras del sitemap devolvían 404 a Google, y la web se veía perfecta.** GitHub Pages no reescribe rutas: para `/pricing` busca `pricing.html` y `pricing/index.html`, y si no hay ninguno sirve `404.html` **con estado HTTP 404**. El workflow copia `index.html → 404.html`, así que la SPA arranca y la persona ve su página — el estado 404 sólo lo ve el rastreador. `gen-seo-pages.js` no generaba fichero para ninguna de las ocho rutas de aplicación y las ocho iban en el sitemap con prioridad 0.85–0.9: `/pricing`, `/education`, `/options`, `/options/strategies`, `/about`, `/contact` y `/legal` (sólo `/` existía). Search Console las marca «Enviada: no encontrada (404)», que resta autoridad al resto del sitemap, y encima **los dos botones verdes de las 1.640 páginas estáticas apuntan a `/pricing`**: 3.280 CTA a un 404. Ningún verificador podía verlo: `check-seo.js` saltaba las rutas de app *por ser* rutas de app y sólo miraba las de dos segmentos o más, y `check-seo-en-vivo.js` muestrea a paso fijo (1.648 URLs ÷ 25 = 65) sobre una lista que empieza justo por esas ocho, así que de las ocho comprobaba **la 0**. Fix: el postbuild escribe el shell en cada ruta de app —en las dos formas, `ruta.html` y `ruta/index.html`— con su título, su descripción y su canonical sacados de las MISMAS claves i18n que usa `useSEO`; las tres premium (`/education`, `/options`, `/options/strategies`) salen del sitemap y llevan `noindex, follow`, igual que ya se había decidido para `/performance`. Y las dos cegueras se cierran: `check-seo.js` exige que toda `<loc>` tenga fichero que servir, y el muestreo en vivo mete siempre las rutas cortas. | 🔴 | ✅ Resuelto (2026-09-05) — **requiere desplegar** |
| BUG-082 | **Seis idiomas anunciaban en el `<title>` de Precios un plan que no existe a un precio que no se cobra.** `seoPricingTitle` decía «Kostenloser Plan und Premium ab 9,99 $/Monat» en de, fr, ru, zh, ja y ar —un 70 % por debajo de los 17 €/mes reales— y los diez idiomas anunciaban un «Plan Gratis» inexistente: `SUBSCRIPTION_PLANS` tiene cuatro planes (17/45/200/500 €) y los cuatro se pagan. Es el `<title>`, o sea **lo que Google enseña en el resultado**, y en la UE un precio anunciado por debajo del cobrado es práctica comercial desleal. `check-precios.py` pasaba en verde porque sólo recorría las claves `<plan>Price`: un verificador que recorre una lista protege esa lista y nada más. Curiosamente `seoPricingDesc` estaba bien en los diez, así que el título y su descripción se contradecían en la misma página. Fix: los diez títulos reescritos reusando la redacción que ya tenía cada idioma en su `seoPricingDesc`, y `check-precios.py` comprueba ahora **cualquier importe pegado a una moneda** en los textos de la página de precios, venga de la clave que venga. | 🔴 | ✅ Resuelto (2026-09-05) |
| BUG-083 | **`useSEO` reintroducía en cada ruta de la SPA el hreflang `?lang=xx` que `public/index.html` había quitado por dañino.** El hook emitía diez `<link rel="alternate" hreflang>` a `?lang=de`, `?lang=ja`… sobre la misma URL, mientras él mismo ponía el canonical de esas URLs en la ruta desnuda: las dos señales se contradecían y gana el canonical. La SPA traduce en cliente, así que `/pricing?lang=de` devuelve byte por byte el mismo HTML que `/pricing`; el porqué estaba escrito con detalle en `index.html` y sólo se había aplicado a la portada. Neto: rastreo gastado en diez URLs que Google ya sabía que eran una, y cero idiomas indexados de más. Fix: el hook mantiene sólo el `x-default` apuntando a la ruta actual. Las que sí tienen una URL por idioma —las 1.640 estáticas, `/de/learn/…`— siguen emitiendo su juego completo. | 🟠 | ✅ Resuelto (2026-09-05) |
| BUG-084 | **Tres defectos de plantilla repetidos en las 1.640 páginas generadas.** (1) `og:locale` salía con el código corto (`es`, `zh`, `ja`) y la especificación de Open Graph pide `language_TERRITORY`: valor inválido que Facebook, LinkedIn y WhatsApp descartan, así que la tarjeta compartida caía al idioma por defecto — `index.html` y `useSEO.js` ya lo hacían bien, sólo el generador no. (2) Las `meta description` se cortaban con `slice(158)` a pelo y acababan a mitad de palabra en **643 de 1.640** («…the buyer pays a p»); Google reescribe el fragmento cuando lo ve truncado, así que el texto escrito con cuidado no se usaba. (3) El `<lastmod>` de las 1.648 URLs era la fecha del build, de modo que cada despliegue le decía a Google que había cambiado el sitio entero; Google documenta que ignora el `lastmod` que no le cuadra, así que el campo gastaba credibilidad sin aportar nada. Fix: mapa `OG_LOCALE`, recorte por palabra, y `lastmod` sacado del último commit de la fuente de cada sección (`mockData.js`, `marketTypesContent.js`, `es.edu.js`…) — con `fetch-depth: 0` en el workflow, y **omitiendo la etiqueta** si no hay historia, en vez de inventar una fecha. | 🟡 | ✅ Resuelto (2026-09-05) |
| BUG-085 | **El manifiesto de la PWA llevaba doce `%PUBLIC_URL%` literales: no era un manifiesto degradado, era uno inválido.** CRA interpola ese marcador **sólo** en `index.html`; el resto de `public/` lo copia tal cual, y nadie lo había notado. Así viajaban a producción `"id": "%PUBLIC_URL%/"`, `"start_url": "%PUBLIC_URL%/?source=pwa"`, `"scope"`, los **seis iconos** y los **tres accesos directos**. Consecuencia: la aplicación no se puede instalar, «Añadir a pantalla de inicio» no tiene icono y Lighthouse falla instalabilidad. No rompe ninguna pantalla —de ahí que llevara tiempo publicado—. Lo destaparon tres auditorías externas que vieron el síntoma (una petición a `/%PUBLIC_URL%/favicon.svg`) y **las tres culparon a `index.html`**, que estaba correcto: el build lo sustituye bien y sirve `/favicon.svg`. Una de ellas llegó a «arreglar» el fichero sano. Fix: rutas relativas a la raíz (`/icon-192.png`), que son independientes del dominio, y un guardián en `check-seo.js` que falla si cualquier fichero del build conserva un `%PUBLIC_URL%` — comprueba `manifest.json`, `sw.js`, `offline.html`, `robots.txt` y las 1.650 páginas. Sólo ese marcador y no `%REACT_APP_*%`: el primero lo resuelve siempre el build, el segundo queda literal a propósito en local. | 🟠 | ✅ Resuelto (2026-09-05) |
| BUG-086 | **`/login`, `/register` y `/forgot-password` estaban en el peor de los dos mundos: 404 e indexables.** No tenían fichero estático, así que GitHub Pages las servía con estado 404 —un enlace de login compartido no abría para ningún rastreador ni previsualizador— y a la vez eran indexables, porque `useSEO` no les pasaba `noindex` y `robots.txt` no las bloquea. Fix: entran en `APP` con estado 200 y `noindex, follow` en el HTML crudo, y `useSEO` declara lo mismo para que el DOM renderizado no contradiga al servido. **No se añaden a `robots.txt` a propósito**: una ruta prohibida en robots nunca llega a leer su propio `noindex`, que es exactamente el error que ya costó el caso `/performance`. | 🟡 | ✅ Resuelto (2026-09-05) |
| BUG-087 | **El despliegue publicaba sin que ningún verificador corriera.** `ci.yml` se dispara con `pull_request`; `deploy-gh-pages.yml`, con `push` a `main`. Un push directo a main —o un merge sin PR— publicaba en producción saltándose lint, i18n, motor, SEO y precios. No es teórico: **pasó el mismo 2026-09-05 con el arreglo del SEO** (BUG-081), que se fusionó a main y desplegó sin que el CI se ejecutara ni una vez. Lo señaló también una auditoría externa (P0-03). Fix: el propio workflow de despliegue ejecuta ahora `eslint`, `i18n-check`, `engine-check`, `check-seo` y `check-precios` **entre el build y la publicación**, así que un artefacto que no pasa no llega a `gh-pages`. Las comprobaciones de higiene del repositorio (mapa, enlaces de doc, asistente) se quedan en `ci.yml`: bloquear un despliegue porque `MAPA.md` está desfasado sería una puerta mal calibrada. Esto **no sustituye** a proteger la rama en GitHub —un push que no toque `frontend/**` ni siquiera dispara este workflow—, pero cierra el camino por el que se publica de verdad. | 🟠 | ✅ Resuelto (2026-09-05) |


| BUG-088 | **Las 1.640 páginas estáticas eran huérfanas: no salía ni un enlace de la aplicación hacia ninguna.** El mayor activo de captación del proyecto sólo era alcanzable por el `sitemap.xml`, que sirve para DESCUBRIR una URL pero **no reparte autoridad**: las 1.640 competían desde cero mientras la portada acumulaba la suya sin repartirla. Entre ellas sólo se citaban seis «relacionadas», y sus migas y su menú apuntaban a `/education`, `/options` y `/dashboard` — es decir, a rutas con muro que además devolvían 404 (BUG-081). El grafo entero drenaba hacia fuera y no entraba nada. Fix en tres piezas: (a) **40 hubs** nuevos, uno por sección e idioma (`/learn/`, `/tools/`, `/markets/`, `/strategies/` y sus `/<idioma>/…`), que listan sus páginas completas con `CollectionPage` + `ItemList`; (b) las 1.680 páginas llevan los cuatro hubs en cabecera y pie, así que cualquiera está a **dos saltos de la portada**; (c) el pie de la SPA enlaza los hubs del idioma activo — con `<a>`, no con `<Link>`, porque son HTML estático y un `Link` los buscaría entre las rutas de React Router. `check-seo.js` comprueba ahora que **ninguna página se queda sin hub que la enlace**, con su sabotaje. | 🟠 | ✅ Resuelto (2026-09-03) — **requiere desplegar el frontend** |



| BUG-089 | **`check-seo.js` leía `robots.txt` ignorando las reglas `Allow`, y el punto ciego de `check-seo-en-vivo.js` estaba escrito en su propio comentario.** (a) La comprobación «el sitemap anuncia una URL que robots.txt prohíbe» sólo recogía los `Disallow` y comparaba prefijos a pelo. Se destapó probando la pareja `Disallow: /options` + `Allow: /options/strategies/` —pensada para dejar fuera la pantalla premium y dentro las 660 fichas públicas—: el verificador denunciaba **660 páginas perfectamente indexables**. Esa pareja acabó no usándose (las rutas premium se resuelven con `noindex`, ver BUG-086), pero el parser estaba mal de todas formas. Un verificador que grita cuando no pasa nada se acaba desactivando, y con él se va la comprobación que sí servía. Ahora resuelve como un rastreador: gana la regla cuyo patrón case más largo, `Allow` o `Disallow`, con `*` y `$`. (b) Además borraba el prefijo de idioma antes de comparar, o sea que trataba `/en/options/x` como si fuera `/options/x`: `robots.txt` casa rutas **literales** y esa prohibición no la aplica ningún rastreador. (c) El muestreo del verificador en vivo saltaba exactamente las URLs que fallaban — ver BUG-081. | 🟠 | ✅ Resuelto (2026-09-03) |
| BUG-090 | **Nueve de los diez idiomas no recibian ni un solo enlace interno: regalaban autoridad y no cobraban ninguna.** Medido con `auditar-visibilidad.js` sobre el build: de las **1.811 paginas de contenido, solo 182 se alcanzaban siguiendo `<a>` desde la portada, y las 182 eran espanolas**. Las otras 1.629 enlazaban *hacia* la portada y hacia `/pricing` y no recibian nada — una valvula de un solo sentido. Lo que lo hacia invisible es que **dos comprobaciones decian la verdad y ninguna veia el problema**: el `hreflang` estaba puesto y era reciproco, pero `<link rel="alternate">` sirve para DESCUBRIR y agrupar traducciones y **no transmite autoridad** (el sitemap, igual); y «huerfana» en `check-seo.js` significa «ningun hub de SU idioma la enlaza», una condicion **local** que se cumplia. Por eso G-44 se cerro el 2026-09-05 afirmando que «toda pagina queda a dos saltos de la portada»: era cierto dentro de cada idioma y falso entre idiomas. Fix: los `alts` que ya se calculaban para el `hreflang` se pintan como **selector de idioma visible** en el pie de las tres plantillas — no inventa datos, son las traducciones que existen, asi que no puede apuntar a un 404. Entrantes medianos: `/learn/` 2 -> 11, `/patterns/` 1 -> 10, `/candles/` 4 -> 13, `/tools/` 8 -> 17, `/markets/` 10 -> 19. Nuevo invariante en `check-seo.js` (mas de una traduccion declarada exige enlazarlas) con su sabotaje en `probar-verificadores.sh`. | 🟠 | ✅ Resuelto (2026-09-06) — **requiere desplegar el frontend** |
| BUG-091 | **El japones llamaba «tres cuervos» al patron alcista: una senal de trading invertida en pantalla.** `threeWhiteSoldiersName` y `threeBlackCrowsName` valian los dos `三羽烏` en `src/lib/i18n/ja.js`. `三羽烏` es *Three Black Crows*, **bajista**; el alcista *Three White Soldiers* es `赤三兵`. Los otros nueve idiomas distinguian los dos patrones correctamente, asi que era un error aislado del japones. No lo destapo ninguna revision de contenido sino la comprobacion de **titulos duplicados dentro del mismo idioma** de `auditar-visibilidad.js`: dos URLs distintas (`/ja/candles/three-black-crows/` y `/ja/candles/three-white-soldiers/`) con el mismo `<title>`. `i18n-check.js` no podia verlo — comprueba que la clave EXISTA, no que diga otra cosa que su hermana. La correccion es coherente con el vocabulario del propio fichero, que ya usa `三兵` para «soldados» en `labPresetSoldier`. | 🔴 | ✅ Resuelto (2026-09-06) |
| BUG-092 | **Las 2.475 paginas estaticas tenian scroll horizontal en movil, por el ultimo enlace de su menu.** A 390 px de viewport, `/ru/learn/` medía 426 px de `scrollWidth` y `/learn/trading-algoritmico/` 404. El culpable es `nav.top`: es un item flex y `min-width:auto` le impide encogerse por debajo de su contenido, asi que sus enlaces no envolvian aunque `header.top .wrap` si tuviera `flex-wrap:wrap`. El que desbordaba era siempre el ultimo rotulo, el mas largo («Estrategias de opciones», «Опционные стратегии»). Google indexa **mobile-first** y el scroll horizontal es un defecto de usabilidad movil que Search Console reporta. No lo veia ningun verificador: `check-seo.js` lee HTML, no calcula layout, y `capturas.js` cubre las pantallas de la SPA, no las 2.475 estaticas. Lo cazo un smoke de navegador escrito para comprobar OTRA cosa —que el selector de idioma nuevo se viera bien—, y la primera sospecha fue que lo causaba ese selector: ocultarlo por CSS y volver a medir demostro que el `scrollWidth` no cambiaba ni un pixel. Fix: `nav.top{min-width:0}` en las tres plantillas. 404 -> 390 y 426 -> 390. | 🟡 | ✅ Resuelto (2026-09-06) |

---

*Actualizado 2026-09-03 — cinco bugs de SEO (BUG-081…085). El más caro, BUG-081: siete de las ocho rutas principales llevaban devolviendo 404 a los buscadores desde siempre, con la web viéndose perfecta. Lo destapó una captura de Yandex que mandó el dueño, no un verificador.*

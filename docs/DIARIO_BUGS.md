...
| BUG-078 | **Ocho sabotajes de `probar-verificadores.sh` no saboteaban nada: el guardián de los guardianes tenía el fallo que existe para cazar.** Los bloques `python - <<'EOF'` llevaban el cuerpo indentado dos espacios para alinearlo con el resto del script. Python leído por `stdin` no admite indentación a nivel de módulo: cada uno moría con `IndentationError`. Y `probar()` ejecuta el sabotaje con la salida descartada y **sin mirar su código de salida**, así que el fichero quedaba intacto, el verificador seguía pasando y el resultado era «SOBREVIVE al sabotaje». Reproducido en una copia limpia de `main`. Fix: los ocho cuerpos dedentados. **Lo que sigue abierto** es la causa de fondo: un sabotaje que no se aplica es indistinguible de un verificador que no verifica, y `probar()` debería exigir que su sabotaje salga con 0. | 🟠 | ✅ Resuelto (2026-08-31) — apunte para `probar()` en G-41 |

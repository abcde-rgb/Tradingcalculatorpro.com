...
| BUG-081 | **Siete de las ocho rutas principales devolvían HTTP 404 a los buscadores, y la web se veía perfecta.** GitHub Pages sólo sirve `index.html` en la RAÍZ; cualquier otra ruta sin fichero físico recibe `404.html` **con estado 404**. El workflow copia ahí el shell del SPA (`cp build/index.html build/404.html`), así que una persona escribe `/pricing`, React arranca y ve la página bien — pero el rastreador ve un **404 y no indexa**. Afectaba a `/pricing`, `/education`, `/options`, `/options/strategies`, `/about`, `/contact` y `/legal`: **las siete anunciadas en `sitemap.xml`**, tres de ellas con prioridad 0.85–0.9, las más altas del sitio después de la portada. Es la explicación de por qué en los buscadores sólo aparecían páginas estáticas profundas y ni la portada de precios ni la academia. **Ningún verificador podía verlo**: `check-seo.js` mira `build/` en disco, donde no hay códigos de estado; y `check-seo-en-vivo.js`, que sí pide las URLs de verdad, muestreaba con `i % paso` sobre 1.648 URLs —paso ~55— así que de las ocho primeras sólo pedía la portada. Su comentario decía que las primeras «son las rutas principales, y son justo las que nunca fallan». Eran las únicas que fallaban. Fix: el `postbuild` escribe un `build/<ruta>/index.html` de verdad para las seis rutas públicas, partiendo del shell compilado y cambiándole título, descripción, canonical y `x-default` — mismo JavaScript, misma aplicación, pero 200. Las tres rutas con muro (`/education`, `/options`, `/options/strategies` son `ProtectedRoute premiumOnly` y mandan a `/login`) **salen del sitemap y entran en `robots.txt`**: publicar un 200 ahí sólo serviría para que Google indexara una pantalla de acceso. Y el muestreo del verificador en vivo pide ahora siempre los ocho primeros y los ocho últimos. 1 sabotaje nuevo. | 🔴 | ✅ Resuelto (2026-09-03) — **requiere desplegar el frontend** |

| BUG-082 | **Las 1.640 páginas estáticas eran huérfanas: no salía ni un enlace de la aplicación hacia ninguna.** El mayor activo de captación del proyecto sólo era alcanzable por el `sitemap.xml`, que sirve para DESCUBRIR una URL pero **no reparte autoridad**: las 1.640 competían desde cero mientras la portada acumulaba la suya sin repartirla. Entre ellas sólo se citaban seis «relacionadas», y sus migas y su menú apuntaban a `/education`, `/options` y `/dashboard` — es decir, a rutas con muro que además devolvían 404 (BUG-081). El grafo entero drenaba hacia fuera y no entraba nada. Fix en tres piezas: (a) **40 hubs** nuevos, uno por sección e idioma (`/learn/`, `/tools/`, `/markets/`, `/strategies/` y sus `/<idioma>/…`), que listan sus páginas completas con `CollectionPage` + `ItemList`; (b) las 1.680 páginas llevan los cuatro hubs en cabecera y pie, así que cualquiera está a **dos saltos de la portada**; (c) el pie de la SPA enlaza los hubs del idioma activo — con `<a>`, no con `<Link>`, porque son HTML estático y un `Link` los buscaría entre las rutas de React Router. `check-seo.js` comprueba ahora que **ninguna página se queda sin hub que la enlace**, con su sabotaje. | 🟠 | ✅ Resuelto (2026-09-03) — **requiere desplegar el frontend** |

| BUG-083 | **La meta description de las 1.640 páginas se cortaba a media palabra, y el buscador la tiraba.** Salía de un `.slice(0, 158)` en los cuatro generadores. En la página rusa de `/ru/learn/operar-noticias/` terminaba en «… сгоревшим счётом. И» — la «И» es el arranque de «Используйте», partido por la mitad. Yandex la descartaba y se inventaba el resumen con el texto de la página; como esas páginas tenían **94 palabras visibles** (unas 40 propias del tema y el resto menú, migas, dos botones y seis enlaces), lo más sustancioso que encontraba era **el descargo legal del pie**, y eso es lo que publicaba como descripción del tema. Reportado por el dueño con una captura de Yandex. Fix: `recortar()` corta en frontera de palabra y no añade puntos suspensivos si el corte cae detrás de un punto. Y el fondo del problema —el contenido delgado— se ataca publicando los **4.300 conceptos** que ya existían traducidos a los diez idiomas en `lib/i18n/*.edu.js` y que sólo se pintaban dentro de la aplicación: las páginas de academia pasan de 94 a ~240 palabras, con `teaches` en el JSON-LD declarado **sólo** cuando los conceptos están impresos. Dos fallos menores del mismo sitio: `og:locale` emitía `ru` donde Open Graph exige `ru_RU`, y **ninguna de las 1.640 declaraba `rel="icon"`** — por eso el sitio salía en Yandex con el globo genérico en vez de su marca. 4 sabotajes nuevos. | 🟠 | ✅ Resuelto (2026-09-03) — **requiere desplegar el frontend** |

| BUG-084 | **`useSEO` deshacía en tiempo de ejecución el arreglo de `hreflang` que `public/index.html` documenta.** El HTML llevaba quince líneas de comentario explicando por qué se habían quitado las diez alternativas `?lang=xx`: la SPA traduce en CLIENTE, así que `/pricing?lang=de` devuelve byte por byte el mismo HTML, Google canonicaliza las diez a una y descarta el resto, y encima se contradicen con el `canonical`, que apunta a la URL desnuda. El arreglo duraba lo que tardaba React en montar: `syncHreflangAlternates()` **borraba las alternativas estáticas** del HTML y las repoblaba con las `?lang=`, en cada ruta y en cada render. Las dos mitades del arreglo se escribieron por separado y nunca se encontraron. Fix: el hook declara sólo `x-default` hacia la propia URL — las páginas que SÍ tienen una URL por idioma son las estáticas, que emiten su juego correcto en el HTML y a las que este hook no llega. Y el canonical pasa a llevar barra final, porque desde BUG-081 quien sirve `/pricing` es un directorio y Pages redirige `/pricing` → `/pricing/`: sin la barra, el canonical apuntaría a una URL que redirige a la que lo declara. | 🟠 | ✅ Resuelto (2026-09-03) — **requiere desplegar el frontend** |

| BUG-085 | **`check-seo.js` leía `robots.txt` ignorando las reglas `Allow`, y el punto ciego de `check-seo-en-vivo.js` estaba escrito en su propio comentario.** (a) La comprobación «el sitemap anuncia una URL que robots.txt prohíbe» sólo recogía los `Disallow` y comparaba prefijos a pelo. Con la pareja `Disallow: /options` + `Allow: /options/strategies/` —la que deja fuera la pantalla premium y dentro las 66 fichas públicas— denunciaba **660 páginas perfectamente indexables**. Un verificador que grita cuando no pasa nada se acaba desactivando, y con él se va la comprobación que sí servía. Ahora resuelve como un rastreador: gana la regla cuyo patrón case más largo, `Allow` o `Disallow`, con `*` y `$`. (b) Además borraba el prefijo de idioma antes de comparar, o sea que trataba `/en/options/x` como si fuera `/options/x`: `robots.txt` casa rutas **literales** y esa prohibición no la aplica ningún rastreador. (c) El muestreo del verificador en vivo saltaba exactamente las URLs que fallaban — ver BUG-081. | 🟠 | ✅ Resuelto (2026-09-03) |

...
| BUG-081 | **El «Monitor de Errores» del panel de admin llevaba desde siempre leyendo una tabla que no escribía nadie.** `admin_routes.py` tiene `GET /admin/errors` y `POST /admin/errors/{id}/resolve` sobre `error_logs`, y `AdminPage.jsx` pinta su tarjeta. Lo que no existía era el otro extremo: el `componentDidCatch` del `ErrorBoundary` hacía `console.error` y un comentario *«Wire up Sentry here when DSN is available»* que nunca se cableó. Una consola en el navegador de otra persona no la lee nadie, así que **`error_logs` no recibió una sola fila en toda la vida del proyecto** y la tarjeta pintaba «✓ Sin errores pendientes» con la misma cara tanto si la web iba fina como si se caía cada dos minutos. Las dos cosas se veían igual, que es lo peor que puede hacer un monitor: no es que no avisara, es que afirmaba que no había nada. Peor aún, los fallos que **nunca** llegan a un `ErrorBoundary` —una promesa rechazada sin `catch`, un error fuera del árbol de React— no los veía nadie por ningún sitio. Fix: `POST /api/errors/report` (público, `30/minute`, todos los campos con tope, tope de 500 huellas abiertas y retención de 120 días) + `reportarError.js` en el frontend, cableado al `ErrorBoundary` y a `unhandledrejection`/`error` desde `index.js`, antes de montar React. Se **agrupa por huella** (tipo + mensaje normalizado + ruta) con contador `veces`: sin eso, un bug en una pantalla con tráfico mete miles de filas idénticas y con `limit=50` tapa todos los demás. **Un error resuelto que vuelve a ocurrir se reabre**, o marcar «resuelto» un bug vivo lo esconde para siempre. **No se guarda `user_id`**: sólo un booleano `autenticado`, así que `error_logs` no contiene dato personal y no entra en las listas del RGPD. El servidor **redacta** correos, tokens JWT y números largos, y tira la query string (ahí viajan los tokens de reseteo). Contador de errores abiertos en la fila de métricas, visible desde todas las secciones. Medido contra Postgres y el backend vivo: dos informes idénticos → una fila con `veces: 2`; `Loading chunk 4821 failed` y `Loading chunk 77 failed` → la misma fila; el correo del mensaje sale como `[correo]`; `?token=SECRETO` no llega a la tabla; `open_errors` 2 → resolver → 1 → el error reaparece → se reabre (`veces: 3`) → 2. 28 tests, 13 sabotajes. | 🟠 | ✅ Resuelto (2026-09-02) — **requiere desplegar** |

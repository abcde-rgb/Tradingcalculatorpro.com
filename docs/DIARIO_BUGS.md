...
| BUG-081 | **CI llevaba en rojo en TODOS los PR, y el único paso que fallaba era el que vigila la CSP.** `ci.yml` compila sin `REACT_APP_BACKEND_URL`. `public/index.html` escribe la política con `%REACT_APP_BACKEND_URL%` y **CRA deja el marcador tal cual cuando la variable no existe** —sólo interpola las `REACT_APP_*` que estén en el entorno—, así que el navegador descartaba esa fuente («contains an invalid source: it will be ignored») y `csp.js` encontraba una violación en cada una de las doce pantallas de la SPA. Las ocho últimas ejecuciones de CI en rojo por esto, con los otros tres jobs y las veinte comprobaciones anteriores en verde: **un rojo permanente que no habla del diff deja de leerse, y con él dejan de leerse las veinte**. Estaba diagnosticado desde el 03-09 en el mensaje de un commit del PR #229 y sin arreglo publicado. **Lo que hace que esto no sea sólo ruido de CI**: el marcador es una de las DOS formas de romperlo, y la otra no deja rastro. Un secreto **definido y vacío** —`${{ secrets.X }}` de un secreto que no existe vale `''`— hace que CRA sustituya por nada: la directiva queda sintácticamente perfecta, sin el backend dentro, y sin un solo aviso en la consola. Eso publica una web que carga entera, se ve bien y **no tiene un `fetch` que llegue a la API** —`API = null` por un lado y la CSP bloqueando por el otro—, que es BUG-067 otra vez y por otra puerta. Fix, cuatro piezas: (a) `ci.yml` compila con `REACT_APP_BACKEND_URL=http://127.0.0.1:8080`, el mismo valor que ya usaban `stack/arriba.sh` y los recompilados de `probar-verificadores.sh` —localhost y no la API real: un runner de GitHub sí tiene red y las sondas acabarían llamando a producción—; (b) **`check-csp-origenes.js`**, verificador nuevo y offline, que en vez de comparar contra un valor esperado exige el par `http(s)://host` + `ws(s)://host` dentro de `connect-src` —ninguna fuente de terceros de la política es un WebSocket, así que el único `ws://` de la lista ES el backend—, corre tras el build en CI y **en el despliegue con `--exigir-todos`**, y de regalo cierra offline el fallo del esquema (en CSP3 `https://host` NO autoriza `wss://host`) que hasta ahora sólo veía un navegador con sesión; (c) `csp.js` daba **verde sobre la SPA creyendo mirar una página estática** —el servidor devuelve `index.html` para toda ruta desconocida y `/en/learn/` es un directorio, no una página—, así que el bloque de la política dura (`default-src 'none'`) llevaba tiempo midiendo la blanda: ahora se exige ver esa directiva en la página servida; (d) el bloque del WebSocket no puede correr sin backend, así que con `--sin-sesion` se dice que queda SIN comprobar y dónde sí se comprueba — y la sonda **falla si se le pasa la bandera con el backend en pie**, para que no se convierta en el verde barato. 4 sabotajes nuevos. | 🟠 | ✅ Resuelto (2026-09-04) |
| BUG-082 | **Los cuatro campos de la calculadora de la portada no tenían etiqueta, y la Academia pintaba con la paleta cruda.** Primera tanda del banco E2E con backend y Postgres reales sobre esta rama; la sonda de accesibilidad (axe, ocho pantallas × dos temas × dos anchos) salió roja. **(a) `critical` ×4**: en `LandingDemoCalculator` el `<Label>` estaba al lado del `<Input>` pero sin `htmlFor` —dos elementos vecinos, no un par—, así que lo primero que la web invita a tocar se anunciaba «edit, blank» cuatro veces a quien navega con lector de pantalla. `useId`, porque el componente se pinta cuatro veces y un id fijo los ataría todos al primero. **(b) `serious`**: el rótulo «Stop» de la regleta con `opacity-70` sobre el rojo de `--short` da **3,58:1** en oscuro y **3,80:1** en claro, bajo el 4,5 de la WCAG 2.1 para texto pequeño; a `opacity-90` sube a 5,21 y 5,51. **(c) `serious` ×3**: `TradingBasicsGuide.jsx` pintaba con `text-green-400`, `text-red-400`, `text-cyan-400`… —paleta elegida para el tema oscuro—, y sobre papel su verde daba **1,67:1**. Los tokens de señal existen justo para esto y ya sustituyeron 704 clases con hexadecimal a mano; este fichero se quedó fuera porque `PENDIENTES.md` lo daba por «escrito y nunca conectado» **y no lo está** (`EducationPage.jsx:1261` lo monta). 67 clases migradas, ninguna cruda. **(d) Y cinco «serious» que no lo eran**: `EmptyState` enseña una muestra difuminada de lo que aparecerá en la tarjeta —`opacity-35`, `blur`, `pointer-events-none`, `aria-hidden`—, pero la regla `color-contrast` de axe empareja por lo que se VE, no por el árbol de accesibilidad, y medía el 35 % de opacidad. La WCAG §1.4.3 exime la decoración pura; la exención que se añade a la sonda pide `aria-hidden` **y** `pointer-events:none` a la vez, sólo se aplica a `color-contrast`, y se cuenta e imprime en vez de descartarse en silencio. Con su sabotaje: devolver `opacity-70` al rótulo del stop tiene que seguir poniéndola roja — comprobado (verde → 2 graves → verde). Resultado: **0 incumplimientos graves** en las 16 combinaciones, escritorio y móvil, y `contraste.js` verde en los dos temas. | 🟠 | ✅ Resuelto (2026-09-04) |

...
| `GET` | `/api/education/chart-pattern-catalog` | CONSTRUIR | Enciclopedia de los 13 patrones chartistas que ya detecta `chart_patterns.py` (G-40). Igual que la fila de arriba, pero para el escáner geométrico nuevo. |
| `GET` | `/api/education/chart-pattern-scan/{symbol}` | CONSTRUIR | Detector geométrico (H-C-H ± inverso, doble/triple techo-suelo, rectángulo, triángulos, cuñas, ensanche) sobre `detect_swings` — Lote 5 §5 de `DETALLE_TECNICAS_IMPLEMENTACION.md`. Cierra media G-40: el escáner llegaba a 35 patrones de vela y 0 chartistas; esto detecta 13 de los ~19-20 (con tests contra el ejemplo de la propia doc — H-C-H 110/120/111 → objetivo 80). Falta la pantalla: overlay de las líneas sobre el gráfico + tarjeta reutilizando `ChartPatternFigure` (§5.5). Los 9 restantes (Cup&Handle, banderas, Darvas…) no están porque su geometría no es un ajuste de swings/trendline y forzarlos aquí habría dado un número con apariencia correcta y sin serlo. |

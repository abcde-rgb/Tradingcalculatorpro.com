...
TODO=("$@")
for OBJETIVO in recorrido analitica nulos temas ticker precio-viejo brokers idiomas accesibilidad autorizacion rgpd persistencia pasarelas; do
  if [ ${#TODO[@]} -gt 0 ]; then
    encontrado=0
  ...
  case "$OBJETIVO" in
    ...
    brokers)      corre "brókers: aviso y afiliación" node "$E2E/navegador/brokers.js" ;;
